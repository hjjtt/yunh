...
1. 为什么要把 POJO 单独拆分成一个模块？
   ...
   POJO 是微服务之间通信的"契约"，它需要被多个服务共享，但又不应该把整个服务的业务逻辑暴露出去。

2. `@RestController` 和 `@Controller` 有什么区别？

   `@RestController` 为一个组合注解，相当于 `@Controller` 和 `@ResponseBody` 的组合

3. 为什么要统一响应格式？直接在 Controller 返回对象不行吗？

   对小项目适用,但是对于大项目来说会很繁琐

4. 父工程的 `<dependencyManagement>` 和 `<dependencies>` 有什么区别？

   <dependencyManagement> 作用 仅声明依赖版本，不实际引入依赖 <dependencies>直接引入依赖，所有子模块自动继承
